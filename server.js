...
io.on("connection", (socket) => {
  ...
  // --- YEH PART FUNCTION KE BAHAR HOGA (Global Monitor) ---

setInterval(async () => {
  const expiredSessions = await AccessSession.find({
    isActive: true,
  ...
  for (const session of expiredSessions) {
    ...
    await session.save();
    io.to(`session_${session.visitorId}`).emit('sessionExpired', { 
        message: "Your 1-hour access has expired!" 
    });
  }
}, 30000);
  // ================= PREMIUM CHAT =================
...
// Jab expiry time aaye, trigger expiry signal
const checkExpiryAndNotify = (userId) => {
  io.to(`session_${userId}`).emit('sessionExpired', { message: "Your access has expired" });
};
